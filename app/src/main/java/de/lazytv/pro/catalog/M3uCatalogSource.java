...
public class M3uCatalogSource {
 ...
 private Catalog loadRemote(Playlist source)throws CatalogException{\n  String exactUrl=source.getUrl();
  ...
  try(Response res=REMOTE.newCall(req).execute()){
   ...
   if(!res.isSuccessful()){if(code==884){Catalog fb=xtreamFallback(source,exactUrl);if(fb!=null)return fb;}throw new CatalogException("Remote M3U HTTP "+code);}
  ...
  }catch(javax.net.ssl.SSLException e){Log.e("LazyTV-M3U","REMOTE_TLS host="+host(exactUrl)+" reason="+safe(e));Catalog fb=xtreamFallback(source,exactUrl);if(fb!=null)return fb;throw new CatalogException("TLS/SSL: "+safe(e),e);}
